...
export default function LoginForm({ onToggleRegister }: LoginFormProps) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (result?.error) {
      toast.error(t.loginError || 'Erreur de connexion. Vérifiez vos identifiants.');
    ...
    <div className="flex flex-col items-center justify-center h-full w-full p-8">
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-xs space-y-5">
        <div> {/* Conteneur pour le label et l'input */}
          <label htmlFor="login-email" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t.loginEmail}</label>
          <div className="relative">
            ...
            <input type="email" id="login-email" value={email} onChange={(e) => setEmail(e.target.value)} required
                   ...
                   placeholder={Array.isArray(t.loginEmailPlaceholder) ? t.loginEmailPlaceholder[0] : t.loginEmailPlaceholder || 'votre.email@exemple.com'} disabled={isSubmitting} />
          </div>
        </div>
        
        <div> {/* Conteneur pour le label et l'input */}
          <label htmlFor="login-password" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t.loginPassword}</label>
          <div className="relative">
            <Lock size={20} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
            <input type="password" id="login-password" value={password} onChange={(e) => setPassword(e.target.value)} required
                   className="w-full pl-10 pr-3 py-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 transition-colors placeholder-gray-400 dark:placeholder-gray-500"
                   placeholder={Array.isArray(t.loginPasswordPlaceholder) ? t.loginPasswordPlaceholder[0] : t.loginPasswordPlaceholder || '********'} disabled={isSubmitting} />
          </div>
          <Link href="#" className="block text-right text-xs text-orange-500 hover:underline mt-1">{t.loginForgotPassword}</Link>
        </div>
        ...
        <button type="submit" disabled={isSubmitting}
          ...
          {isSubmitting ? t.loginLoading || 'Connexion en cours...' : t.loginButton || 'Se connecter'}
        </button>
      </form>
      ...
      <div className="relative flex items-center w-full max-w-xs my-6">
        ...
        <span className="flex-shrink mx-4 text-gray-400 dark:text-gray-500 text-sm">{t.loginWith}</span>
        <div className="flex-grow border-t border-gray-300 dark:border-gray-600"></div>
      </div>
