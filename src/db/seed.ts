...
import { books, values } from './schema';
...
async function insertInitialValues() {
  ...
  for (const name_fr of baseValues) {
    await db.insert(values).values({ name_fr }).onConflictDoNothing();
  }
...
async function migrateValuesToMultilang() {
  ...
  const allValues = await db.select().from(values);
