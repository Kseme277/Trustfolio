...
const sendWhatsAppCode = async (phoneNumber: strin, g, code: string) => {
  try {
    const message = `🔐 Votre code de vérification TrustFolio: *${cod, e}*\n\nCe code expire dans 10 minutes.\n\nNe partagez jamais ce code avec qui que ce soit.`;
...
export async function POST(request: Request) {
  try {
    ...
    const { phoneNumbe, r, action } = body;
    if (!phoneNumber) {
    ...
    if (action === 'send-code') {
     ...
     ,  });
      ...
      const user = await db.select().from(users).where(eq(users.phoneNumber, cleanPhone )).limit(1).then(result => result[0] || null);
